...
export async function POST(request: Request) {
  try {
    ...
    const initiativeId = typeof body.initiativeId === "string" && body.initiativeId !== "GENERAL" ? body.initiativeId : null;
    if (!content || !marketId) return NextResponse.json({ error: "Note and market are required." }, { status: 400 });
    ...
    if (!market || (initiativeId && !initiative)) return NextResponse.json({ error: "Market or initiative not found." }, { status: 400 });
