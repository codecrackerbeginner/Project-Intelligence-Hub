...
export function InitiativeInput({ initiatives, fixedInitiative, compact = false }: Props) {
  ...
  return (
    <div className="relative">
      <Button type="button" size={compact ? "sm" : "default"} onClick={() => setOpen((v) => !v)} className="bg-[#78FAAE] text-[#0D3B32] hover:bg-[#78FAAE]/90">
        <FileUp className="h-4 w-4" />Input
      </Button>
      {open ? (
        <div className="absolute right-0 top-11 z-40 w-[min(34rem,calc(100vw-3rem))] rounded-xl border border-[#78FAAE] bg-card p-5 text-left shadow-xl">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-1.5 text-sm">
              ...
              <label className="flex min-h-24 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed bg-secondary/20 px-4 py-5 text-center transition-colors hover:border-[#78FAAE] hover:bg-[#78FAAE]/5">
                <UploadCloud className="mb-2 h-5 w-5 text-[#0D3B32]" />
                <span className="font-medium">Choose file</span>
                <span className="mt-1 text-xs text-muted-foreground">PPT, PDF, Excel, Word or TXT</span>
                <input type="file" className="sr-only" accept={ACCEPTED_EXTENSIONS.join(",")} disabled={uploading} onChange={handleFileChange} />
              </label>
